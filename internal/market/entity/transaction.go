...
func NewTrasaction(sellingOrder *Order, buyingOder *Order, shares int, price float64) *Transaction {
	total := float64(shares) * price

	return &Transaction{
		ID:           uuid.New().String(),
		SellingOrder: sellingOrder,
		BuyingOrder:  buyingOder,
		Shares:       shares,
		Price:        price,
		Total:        total,
		DateTime:     time.Now(),
...
func (t *Transaction) AddBuyOrderPendingShares(shares int) {
	t.BuyingOrder.AddPendingShares(shares)
}

func (t *Transaction) AddSellOrderPendingShares(shares int) {
	t.SellingOrder.AddPendingShares(shares)
}
